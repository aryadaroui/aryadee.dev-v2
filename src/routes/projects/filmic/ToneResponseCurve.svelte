<script lang="ts">
	...
	function dragged(event: DragEvent, d: number[]) {
		...
		tone_response_curve[d[0]][1] = slider_value;
		update_line("#tone-response-curve-interp-line", tone_response_curve);
	}
	...
	// Initialize //  //  //  //  //  //  //  //  //  //  //  //
	export var tone_response_curve;
	...
	onMount(() => {
		const graph = d3
			.select("#tone-response-curve-graph")
			.append("svg")
		...
		graph
			.append("text")
			.attr("class", "tone-response-curve-graph-y-label")
		...
		graph
			...
			.attr("class", "tone-response-curve-handle")
			// @ts-ignore
			.call(drag);

		graph
			.append("path")
			.datum(tone_response_curve)
			.attr("id", "tone-response-curve-interp-line")
...
<div id="tone-response-curve-container">
	<div id="tone-response-curve-graph" />
</div>
...
<style>
	...
	:global(.tone-response-curve-handle) {
		transition: stroke-width 0.2s;
	}

	:global(.tone-response-curve-handle:hover) {
		stroke-width: 2;
		stroke: white;
		cursor: grab;
	}

	:global(.tone-response-curve-handle:active) {
		cursor: grabbing;
	}
